...
\node (ExtensionHtml) [package, right=of Module]  { 
AdminDSS \\
...
\node (FlowControl) [package, below=of Module]  { 
AST \\
ActualArgsTree \\
AssignTree \\
BlockTree \\
...
\node (Intrinsics) [package, right=of FlowControl]  { 
DSSLibrary \\
DateLibrary \\
IsLibrary \\
LengthAndWithinLibrary \\
...
\node (State) [package, right=of Intrinsics]  { 
DSSEvaluator \\
DSSExecutionContext \\
DSSFunction \\
Evaluator \\
...
\node (Value) [package, right=of State]  { 
DSSValue \\
DSSValueBool \\
DSSValueDate \\
DSSValueFloat \\
...
\node (AST) [package, below=of FlowControl]  { 
AST \\
ActualArgsTree \\
AssignTree \\
BlockTree \\
...
\node (Lexer) [package, right=of AST]  { 
Lexer \\
SourceReader \\
Symbol \\
Token \\
...
\node (Parser) [package, right=of Lexer]  { 
Parser
} ;
\node [tab] at (Parser.north west)
{ parser } ;

\node (Visitor) [package, right=of Parser]  { 
ASTVisitor \\
PrintVisitor
} ;
...
\path [utilizes] (Module) -- (FlowControl) {};
